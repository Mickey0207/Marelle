...
function makeSupabase(c, accessToken) {
...
function setAuthCookies(c, accessToken, expiresInSec, refreshToken) {
...
// ../backend/API/categories/index.ts
init_modules_watch_stub();
var app3 = new Hono2();
var ACCESS_COOKIE3 = "sb-access-token";
var ADMIN_SESSION_COOKIE3 = "admin-session";
function makeSvc2(c) {
  const url = c.env.SUPABASE_URL;
  const key = c.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !key) throw new Error("Server misconfigured");
...
__name(makeSvc2, "makeSvc");
async function requireAuth2(c) {
  const access = getCookie(c, ACCESS_COOKIE3);
  if (access) return true;
  const sess = getCookie(c, ADMIN_SESSION_COOKIE3);
  return !!sess;
}
...
__name(buildCategoryTree, "buildCategoryTree");
...
app3.post("/backend/categories", async (c) => {
  if (!await requireAuth2(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const body = await c.req.json();
...
app3.patch("/backend/categories/:id", async (c) => {
  if (!await requireAuth2(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const id = c.req.param("id");
    const body = await c.req.json();
...
app3.delete("/backend/categories/:id", async (c) => {
  if (!await requireAuth2(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const id = c.req.param("id");
    const svc2 = makeSvc2(c);
...
app3.post("/backend/categories/:id/upload-image", async (c) => {
  if (!await requireAuth2(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const id = c.req.param("id");
...
// ../backend/API/products/index.ts
init_modules_watch_stub();
var app4 = new Hono2();
var ACCESS_COOKIE4 = "sb-access-token";
var ADMIN_SESSION_COOKIE4 = "admin-session";
function makeSvc3(c) {
  const url = c.env.SUPABASE_URL;
  const key = c.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !key) throw new Error("Server misconfigured");
...
__name(makeSvc3, "makeSvc");
async function requireAuth3(c) {
  const access = getCookie(c, ACCESS_COOKIE4);
  if (access) return true;
  const sess = getCookie(c, ADMIN_SESSION_COOKIE4);
  return !!sess;
}
...
app4.get("/backend/products/:id", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const id = c.req.param("id");
    const svc2 = makeSvc3(c);
...
app4.post("/backend/products", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const body = await c.req.json();
    const svc2 = makeSvc3(c);
...
app4.patch("/backend/products/:id", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const id = c.req.param("id");
    const body = await c.req.json();
...
app4.delete("/backend/products/:id", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const id = c.req.param("id");
    const svc2 = makeSvc3(c);
...
app4.post("/backend/products/:id/photos/batch", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const id = c.req.param("id");
    const body = await c.req.json();
...
app4.patch("/backend/products/:id/photos", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const id = c.req.param("id");
    const body = await c.req.json();
...
app4.get("/backend/products/:productId/inventory", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
...
app4.post("/backend/products/:productId/inventory", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const productId = c.req.param("productId");
    const body = await c.req.json();
...
app4.patch("/backend/products/:productId/inventory/:inventoryId", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const productId = c.req.param("productId");
    ...
    if (body.barcode !== void 0) allowed.barcode = body.barcode || null;
    if (body.hs_code !== void 0) allowed.hs_code = body.hs_code || null;
    if (body.origin !== void 0) allowed.origin = body.origin || null;
    if (body.notes !== void 0) allowed.notes = body.notes || null;
    if (body.weight !== void 0) allowed.weight = body.weight ? parseFloat(body.weight) : null;
    if (body.length_cm !== void 0) allowed.length_cm = body.length_cm ? parseFloat(body.length_cm) : null;
    if (body.width_cm !== void 0) allowed.width_cm = body.width_cm ? parseFloat(body.width_cm) : null;
    if (body.height_cm !== void 0) allowed.height_cm = body.height_cm ? parseFloat(body.height_cm) : null;
    if (body.sku_level_1 !== void 0) allowed.sku_level_1 = body.sku_level_1 || null;
    if (body.sku_level_2 !== void 0) allowed.sku_level_2 = body.sku_level_2 || null;
    if (body.sku_level_3 !== void 0) allowed.sku_level_3 = body.sku_level_3 || null;
    if (body.sku_level_4 !== void 0) allowed.sku_level_4 = body.sku_level_4 || null;
    if (body.sku_level_5 !== void 0) allowed.sku_level_5 = body.sku_level_5 || null;
    if (body.sku_level_1_name !== void 0) allowed.sku_level_1_name = body.sku_level_1_name || null;
    if (body.sku_level_2_name !== void 0) allowed.sku_level_2_name = body.sku_level_2_name || null;
    if (body.sku_level_3_name !== void 0) allowed.sku_level_3_name = body.sku_level_3_name || null;
    if (body.sku_level_4_name !== void 0) allowed.sku_level_4_name = body.sku_level_4_name || null;
    if (body.sku_level_5_name !== void 0) allowed.sku_level_5_name = body.sku_level_5_name || null;
    if (body.spec_level_1_name !== void 0) allowed.spec_level_1_name = body.spec_level_1_name || null;
    if (body.spec_level_2_name !== void 0) allowed.spec_level_2_name = body.spec_level_2_name || null;
...
app4.delete("/backend/products/:productId/inventory/:inventoryId", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const productId = c.req.param("productId");
    const inventoryId = c.req.param("inventoryId");
...
app4.post("/backend/products/:productId/variant-photos/:inventoryId/upload", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const productId = c.req.param("productId");
    const inventoryId = c.req.param("inventoryId");
...
app4.patch("/backend/products/:productId/variant-photos/:inventoryId", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const productId = c.req.param("productId");
    const inventoryId = c.req.param("inventoryId");
...
app4.get("/backend/products/:productId/prices", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const productId = c.req.param("productId");
    const svc2 = makeSvc3(c);
...
app4.post("/backend/products/:productId/prices", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const productId = c.req.param("productId");
    const body = await c.req.json();
...
app4.patch("/backend/products/:productId/prices/:priceId", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const productId = c.req.param("productId");
    ...
    if (body.cost_price !== void 0) allowed.cost_price = body.cost_price ? parseFloat(body.cost_price) : null;
    if (body.gold_member_price !== void 0) allowed.gold_member_price = body.gold_member_price ? parseFloat(body.gold_member_price) : null;
...
app4.delete("/backend/products/:productId/prices/:priceId", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const productId = c.req.param("productId");
    const priceId = c.req.param("priceId");
...
app4.post("/backend/products/:id/storage-upload", async (c) => {
  if (!await requireAuth3(c)) return c.json({ error: "Unauthorized" }, 401);
  try {
    const id = c.req.param("id");
    const formData = await c.req.formData();
...
var ACCESS_COOKIE5 = "sb-access-token";
...
function setAuthCookies2(c, accessToken, expiresInSec, refreshToken) {
  ...
  setCookie(c, ACCESS_COOKIE5, accessToken, {
    httpOnly: true,
    secure: !isLocal,
    sameSite: "Lax",
    path: "/",
...
app5.post("/frontend/auth/register", async (c) => {
  try {
    const body = await c.req.json();
...
app5.post("/frontend/auth/login", async (c) => {
  try {
    const body = await c.req.json();
    const { email, password } = body || {};
    if (!email || !password) return c.json({ error: "Missing email or password" }, 400);
...
app5.get("/frontend/auth/me", async (c) => {
  try {
...
app5.post("/frontend/auth/logout", async (c) => {
  try {
    deleteCookie(c, ACCESS_COOKIE5, { path: "/" });
...
app5.get("/frontend/account/profile", async (c) => {
  try {
    const serviceKey = c.env.SUPABASE_SERVICE_ROLE_KEY;
    if (!serviceKey) return c.json({ error: "Server misconfigured" }, 500);
    const svc2 = createClient(c.env.SUPABASE_URL, serviceKey, { auth: { persistSession: false } });
...
app5.post("/frontend/auth/password/reset", async (c) => {
  try {
    const serviceKey = c.env.SUPABASE_SERVICE_ROLE_KEY;
    if (!serviceKey) return c.json({ error: "Server misconfigured" }, 500);
    const svc2 = createClient(c.env.SUPABASE_URL, serviceKey, { auth: { persistSession: false } });
...
app5.patch("/frontend/account/profile", async (c) => {
  try {
...
app5.get("/frontend/account/line/status", async (c) => {
  try {
    const serviceKey = c.env.SUPABASE_SERVICE_ROLE_KEY;
    if (!serviceKey) return c.json({ error: "Server misconfigured" }, 500);
    const svc2 = createClient(c.env.SUPABASE_URL, serviceKey, { auth: { persistSession: false } });
...
app5.post("/frontend/account/line/unbind", async (c) => {
  try {
    const serviceKey = c.env.SUPABASE_SERVICE_ROLE_KEY;
    if (!serviceKey) return c.json({ error: "Server misconfigured" }, 500);
    const svc2 = createClient(c.env.SUPABASE_URL, serviceKey, { auth: { persistSession: false } });
...
app5.get("/frontend/account/line/start", async (c) => {
...
app5.get("/frontend/auth/line/start", async (c) => {
  const { clientId, clientSecret } = getFrontendLineCreds(c);
  if (!clientId || !clientSecret) return c.json({ error: "LINE not configured" }, 500);
  const base = getRedirectBase(c);
  const redirectUri = `${base}/frontend/line/callback`;
...
app5.get("/frontend/line/callback", async (c) => {
  try {
    const { clientId, clientSecret } = getFrontendLineCreds(c);
...
var auth_default2 = app5;

// ../fronted/API/addresses/index.ts
init_modules_watch_stub();
var app6 = new Hono2({ strict: false });
app6.use("*", cors({ origin: /* @__PURE__ */ __name((o) => o || "*", "origin"), allowHeaders: ["Content-Type"], allowMethods: ["GET", "POST", "PATCH", "DELETE", "OPTIONS"], credentials: true }));
var ACCESS_COOKIE6 = "sb-access-token";
...
async function resolveUid(c) {
  const access = getCookie(c, ACCESS_COOKIE6);
  if (access) {
    try {
...
app6.get("/frontend/account/addresses", async (c) => {
  try {
...
app6.post("/frontend/account/addresses", async (c) => {
  try {
    const uid = await resolveUid(c);
    if (!uid) return c.json({ error: "Unauthorized" }, 401);
    const body = await c.req.json().catch(() => ({}));
...
app6.patch("/frontend/account/addresses/:id", async (c) => {
  try {
    const uid = await resolveUid(c);
    if (!uid) return c.json({ error: "Unauthorized" }, 401);
    const id = c.req.param("id");
...
app6.delete("/frontend/account/addresses/:id", async (c) => {
  try {
    const uid = await resolveUid(c);
    if (!uid) return c.json({ error: "Unauthorized" }, 401);
    const id = c.req.param("id");
...
app6.get("/frontend/account/zip/:zip3", async (c) => {
  try {
...
app7.post("/frontend/account/ecpay/map/start", buildStartHtml);
app7.get("/frontend/account/ecpay/map/start", buildStartHtml);
app7.post("/frontend/account/ecpay/map/return", async (c) => {
