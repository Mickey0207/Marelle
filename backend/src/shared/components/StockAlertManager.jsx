...
const StockAlertManager = ({ 
    ...
    products.forEach(product => {
      ...
      if (totalStock <= outOfStockThreshold) {
        newAlerts.push({
          ...
          message: `?ÜÂ? "${product.name}" Â∑≤Áº∫Ë≤®`,
          timestamp: new Date(),
        ...
        newAlerts.push({
          ...
          product,
          stockLevel: totalStock,
          message: `?ÜÂ? "${product.name}" Â∫´Â?‰∏çË∂≥ÔºåÂâ©È§?${totalStock} ‰ª∂`,
          timestamp: new Date(),
  ...
  const getActionText = (action) => {
    const texts = {
      restock: 'Ë£úË≤®',
      deactivate: '‰∏ãÊû∂',
      'notify-supplier': '?öÁü•‰æõÊ???,
      'adjust-threshold': 'Ë™øÊï¥?æÂÄ?,
      dismiss: 'ÂøΩÁï•'
    };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900 font-chinese">Â∫´Â?Ë≠¶Â?</h3>
          <div className="flex items-center space-x-2">
            {alertsByType['out-of-stock'] > 0 && (
              <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">
                <XCircleIcon className="w-3 h-3 mr-1" />
                Áº∫Ë≤® {alertsByType['out-of-stock']}
              </span>
            )}
            {alertsByType['low-stock'] > 0 && (
              <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                <ExclamationTriangleIcon className="w-3 h-3 mr-1" />
                ‰ΩéÂ∫´Â≠?{alertsByType['low-stock']}
              </span>
            )}
            {alerts.length === 0 && (
        ...
        {alerts.length > 0 ? (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600 font-chinese mt-1">
                          ?ÜÂ?ID: {alert.product.id} | Â∫´Â?: {alert.stockLevel} ‰ª?
                        </p>
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${config.badgeColor}`}>
                      {alert.type === 'out-of-stock' ? 'Áº∫Ë≤®' : '‰ΩéÂ∫´Â≠?}
                    </span>
                  </div>
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-600 font-chinese">?Ä?âÂ??ÅÂ∫´Â≠òÊ≠£Â∏?/p>
          </div>
        )}
      </div>
      ...
      {/* ?öÁü•Ë®≠Â? */}
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4 font-chinese">Â∫´Â?Ë≠¶Â?Ë®≠Â?</h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* ?æÂÄºË®≠ÂÆ?*/}
          <div className="space-y-4">
            <h4 className="font-medium text-gray-900 font-chinese">Ë≠¶Â??æÂÄ?/h4>
          ...
          {/* ?öÁü•Ë®≠Â? */}
          <div className="space-y-4">
            <h4 className="font-medium text-gray-900 font-chinese">?öÁü•?πÂ?</h4>
            
            <div className="space-y-3">
              {Object.entries(notificationSettings).map(([key, value]) => {
                const labels = {
                  lowStockEmail: '‰ΩéÂ∫´Â≠òÈÉµ‰ª∂ÈÄöÁü•',
                  outOfStockEmail: 'Áº∫Ë≤®?µ‰ª∂?öÁü•',
                  lowStockPush: '‰ΩéÂ∫´Â≠òÊé®?≠ÈÄöÁü•',
                  outOfStockPush: 'Áº∫Ë≤®?®Êí≠?öÁü•'
                };
      ...
      {alerts.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4 font-chinese">?πÈ??ç‰?</h3>
