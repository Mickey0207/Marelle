...
const UserSegmentManagement = () => {
  ...
  const loadSegmentData = async () => {
    try {
      ...
      console.error('ËºâÂÖ•?ÜÁæ§?∏Ê?Â§±Ê?:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      const mockDetails = {
        ...
        recentActivity: Array.from({ length: 10 }, () => ({
          ...
          action: ['?èË¶Ω?ÜÂ?', '?†ÂÖ•Ë≥ºÁâ©Ëª?, 'ÂÆåÊ?Ë≥ºË≤∑', '?úÂ?', '?•Á?È°ûÂà•'][Math.floor(Math.random() * 5)],
        ...
        demographicBreakdown: {
          ...
          genderDistribution: [
            { gender: 'Â•≥ÊÄ?, percentage: 68, count: 680 },
            { gender: '?∑ÊÄ?, percentage: 32, count: 320 }
          ]
        },
        behaviorPatterns: {
          preferredShoppingTime: [
            { time: '?©‰? (8-12)', percentage: 20 },
            { time: '‰∏ãÂ? (12-18)', percentage: 45 },
            { time: '?ö‰? (18-24)', percentage: 35 }
          ],
          devicePreference: [
            { device: '?ãÊ?', percentage: 60 },
            { device: 'Ê°åÊ?', percentage: 30 },
            { device: 'Âπ≥Êùø', percentage: 10 }
          ]
        }
      };
      setSegmentDetails(mockDetails);
    } catch (error) {
      console.error('ËºâÂÖ•?ÜÁæ§Ë©≥Ê?Â§±Ê?:', error);
    }
  };
  ...
  const tabs = [
    ...
    { id: 'behavior', name: 'Ë°åÁÇ∫?ÜÊ?', icon: EyeIcon },
    { id: 'preferences', name: '?èÂ•Ω?ÜÊ?', icon: FunnelIcon },
    { id: 'personalization', name: '?ã‰∫∫?ñË®≠ÂÆ?, icon: AdjustmentsHorizontalIcon }
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">?®Êà∂?ÜÁæ§ÁÆ°Á?</h1>
          <p className="text-gray-600 mt-1">?ÜÊ??®Êà∂Ë°åÁÇ∫Ê®°Â?‰∏¶ÂâµÂª∫ÂÄã‰∫∫?ñÈ?È©?/p>
        </div>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">{segment.userCount.toLocaleString()} ?®Êà∂</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">Âπ≥Â??ÉË©±?ÇÈï∑</span>
              ...
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">ËΩâÊ???/span>
                <span className="text-sm font-medium text-gray-900">
                  {formatPercentage(segment.behaviorMetrics.conversionRate)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-gray-600">Âπ≥Â?Ë®ÇÂñÆ?πÂÄ?/span>
                <span className="text-sm font-medium text-gray-900">
            ...
            {/* È°ûÂà•?èÂ•Ω?êË¶Ω */}
            <div className="mt-4">
              <p className="text-xs text-gray-500 mb-2">?±È?È°ûÂà•</p>
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-gray-600">{selectedSegment.userCount.toLocaleString()} ?çÁî®??/p>
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div className="bg-white/50 rounded-lg p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-gray-600">Ë∑≥Âá∫??/p>
                  ...
                  <div className="bg-white/50 rounded-lg p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-gray-600">ËΩâÊ???/p>
                  ...
                  <div className="bg-white/50 rounded-lg p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-medium text-gray-600">Âπ≥Â?Ë®ÇÂñÆ</p>
                ...
                {segmentDetails && (
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div className="bg-white/50 rounded-lg p-4">
                      <h4 className="text-md font-medium text-gray-900 mb-4">Âπ¥ÈΩ°?Ü‰?</h4>
                      <div className="space-y-3">
                        {segmentDetails.demographicBreakdown.ageGroups.map((group) => (
                          <div key={group.range} className="flex items-center justify-between">
                            <span className="text-sm text-gray-700">{group.range} Ê≠?/span>
                    ...
                    <div className="bg-white/50 rounded-lg p-4">
                      <h4 className="text-md font-medium text-gray-900 mb-4">Ë®≠Â??èÂ•Ω</h4>
                      <div className="space-y-3">
            ...
            {activeTab === 'behavior' && segmentDetails && (
              <div className="space-y-6">
                {/* ?ÄËøëÊ¥ª??*/}
                <div>
                  <h4 className="text-md font-medium text-gray-900 mb-4">?ÄËøëÁî®?∂Ê¥ª??/h4>
                  <div className="bg-white/50 rounded-lg p-4">
                      ...
                      {segmentDetails.recentActivity.map((activity, index) => (
                        <div key={index} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
                          <div className="flex items-center space-x-3">
                            ...
                            <span className="text-sm text-gray-700">?®Êà∂ {activity.userId.slice(-6)} {activity.action}</span>
                          </div>
                ...
                {/* Ë≥ºÁâ©?ÇÊÆµ?èÂ•Ω */}
                <div>
                  <h4 className="text-md font-medium text-gray-900 mb-4">Ë≥ºÁâ©?ÇÊÆµ?èÂ•Ω</h4>
                  <div className="bg-white/50 rounded-lg p-4">
                    <div className="space-y-3">
            ...
            {activeTab === 'preferences' && (
              <div className="space-y-6">
                <div>
                  <h4 className="text-md font-medium text-gray-900 mb-4">È°ûÂà•?èÂ•Ω?íÂ?</h4>
                  <div className="bg-white/50 rounded-lg p-4">
                    <div className="space-y-3">
            ...
            {activeTab === 'personalization' && (
              <div className="space-y-6">
                <div className="bg-white/50 rounded-lg p-6">
                  <h4 className="text-md font-medium text-gray-900 mb-4">?ã‰∫∫?ñÁ??•Ë®≠ÂÆ?/h4>
                  <div className="space-y-4">
                    <div className="flex items-center justify-between p-4 bg-white/70 rounded-lg">
                      <div>
                        <h5 className="font-medium text-gray-900">?∫ËÉΩ?ÜÂ??®Ëñ¶</h5>
                        <p className="text-sm text-gray-600">?∫ÊñºÊ≠§Â?Áæ§Á??èÂ•Ω?™Â??®Ëñ¶?ÜÂ?</p>
                      </div>
                    ...
                    <div className="flex items-center justify-between p-4 bg-white/70 rounded-lg">
                      <div>
                        <h5 className="font-medium text-gray-900">?ã‰∫∫?ñÈõªÂ≠êÂ†±</h5>
                        <p className="text-sm text-gray-600">?ºÈÄÅÈ?Â∞çÊ≠§?ÜÁæ§?ÑÂ?Â±¨ÂÑ™?†Â??ßÂÆπ</p>
                      </div>
                      <div className="flex items-center">
                        <input
                          type="checkbox"
                    ...
                    <div className="flex items-center justify-between p-4 bg-white/70 rounded-lg">
                      <div>
                        <h5 className="font-medium text-gray-900">?ïÊ??πÊ†ºÈ°ØÁ§∫</h5>
                        <p className="text-sm text-gray-600">?πÊ??®Êà∂Ë°åÁÇ∫Ë™øÊï¥?πÊ†ºÈ°ØÁ§∫Á≠ñÁï•</p>
                      </div>
                      <div className="flex items-center">
                        <input
                          type="checkbox"
                    ...
                    <div className="flex items-center justify-between p-4 bg-white/70 rounded-lg">
                      <div>
                        <h5 className="font-medium text-gray-900">?ã‰∫∫?ñÈ???/h5>
                        <p className="text-sm text-gray-600">?™Ë?È¶ñÈ??ßÂÆπ?åÈ??•Ê?Â∫?/p>
                      </div>
                      <div className="flex items-center">
                        <input
                          type="checkbox"
