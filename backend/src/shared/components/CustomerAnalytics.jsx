...
const CustomerAnalytics = () => {
  ...
  const tabOptions = [
    ...
    { value: 'rfm', label: 'RFM?��?', icon: FunnelIcon },
    { value: 'lifecycle', label: '?�命?��?', icon: ClockIcon },
    { value: 'behavior', label: '購買行為', icon: ShoppingBagIcon },
    { value: 'satisfaction', label: '滿�?度�???, icon: HeartIcon }
  ];
  ...
  const mockCustomerMetrics = {
    totalCustomers: { value: 12580, trend: 8.5, label: '總客?�數' },
    newCustomers: { value: 1250, trend: 15.2, label: '?�客?? },
    activeCustomers: { value: 8950, trend: 5.8, label: '活�?客戶' },
    churned: { value: 520, trend: -12.3, label: '流失客戶' },
    avgLifetimeValue: { value: 8650, trend: 12.1, label: '平�??�命?��??��? },
    avgOrderValue: { value: 2100, trend: 3.7, label: '平�?訂單?��? },
    repeatRate: { value: 35.8, trend: 6.2, label: '?�購?? },
    satisfaction: { value: 4.2, trend: 2.4, label: '滿�?度�??? }
  };
  ...
  const mockLifecycleStages = [
    { stage: '潛在客戶', count: 5230, conversion: 15.8, avgTime: '7�? },
    { stage: '?�客??, count: 3144, conversion: 45.2, avgTime: '30�? },
    { stage: '活�?客戶', count: 8950, conversion: 72.5, avgTime: '90�? },
    { stage: '忠�?客戶', count: 2512, conversion: 85.3, avgTime: '180�? },
    { stage: '?��?客戶', count: 1256, conversion: 92.1, avgTime: '365�?' }
  ];

  const mockCustomerBehavior = [
    { behavior: '?�覽?��?', percentage: 100, sessions: 45230 },
    { behavior: '?�入購物�?, percentage: 25.8, sessions: 11669 },
    { behavior: '?��?結帳', percentage: 12.5, sessions: 5654 },
    { behavior: '完�?付款', percentage: 8.9, sessions: 4025 },
    { behavior: '?��?購買', percentage: 3.2, sessions: 1447 }
  ];

  const mockSatisfactionData = [
    { aspect: '?��??�質', score: 4.5, responses: 2340 },
    { aspect: '?�送�???, score: 4.2, responses: 2156 },
    { aspect: '客�?體�?', score: 4.3, responses: 1890 },
    { aspect: '網�?體�?', score: 4.1, responses: 2567 },
    { aspect: '?�格?��???, score: 3.9, responses: 2234 }
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white/70 backdrop-blur-sm border border-white/50 rounded-xl p-6 shadow-sm">
          ...
          <div className="h-64 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-500 text-sm">客戶?��??�表</p>
        ...
        <div className="bg-white/70 backdrop-blur-sm border border-white/50 rounded-xl p-6 shadow-sm">
          ...
          <div className="space-y-4">
            {[
              { region: '?��?�?, customers: 3250, percentage: 25.8 },
              { region: '?��?�?, customers: 2890, percentage: 23.0 },
              { region: '?�中�?, customers: 1560, percentage: 12.4 },
              { region: '高�?�?, customers: 1340, percentage: 10.7 },
              { region: '?��?�??', customers: 3540, percentage: 28.1 }
            ].map((region, index) => (
          ...
          {mockRFMSegments.map((segment, index) => (
            <div key={index} className={`${segment.bgColor} border border-gray-200 rounded-xl p-6`}>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">佔�?</span>
                  <span className="font-bold text-gray-900">{formatPercentage(segment.percentage)}</span>
                </div>
                
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">RFM評�?</span>
            ...
            <div key={index} className="p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center space-x-4 text-sm">
                  <span className="text-gray-600">轉�??? {formatPercentage(stage.conversion)}</span>
                  <span className="text-gray-600">平�??��?: {stage.avgTime}</span>
                </div>
              </div>
            ...
            <div key={index} className="p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-xs text-gray-500">({formatNumber(item.responses)}?��???</span>
                </div>
              </div>
      ...
      <div className="w-full px-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8">
          <div className="mb-4 lg:mb-0">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">客戶?��?</h1>
            <p className="text-gray-600">深度了解客戶行為，�??�客?�價?��?忠�?�?/p>
          </div>
