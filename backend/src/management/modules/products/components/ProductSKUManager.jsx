...
const ProductSKUManager = ({ variants, skus, setSKUs, formData }) => {
  ...
  const handleGenerateAllSKUs = () => {
    ...
    try {
      ...
      setSKUs(newSKUs);
      alert(`分析分析 ${newSKUs.length} ?�SKU`);
    } catch (error) {
      console.error('分析SKU失分析:', error);
      alert('分析SKU失分析');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="sku-manager space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-gray-900 font-chinese">SKU 管分析</h3>
      ...
      <div className="glass rounded-lg overflow-visible">
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gradient-to-r from-[#cc824d] to-[#b3723f] text-white">
                ...
                <th className="px-4 py-3 text-left font-chinese">SKU分析��</th>
                <th className="px-4 py-3 text-left font-chinese">變分析組分析</th>
                <th className="px-4 py-3 text-left font-chinese">?�本??/th>
                <th className="px-4 py-3 text-left font-chinese">?�價</th>
                <th className="px-4 py-3 text-left font-chinese">分析??/th>
                <th className="px-4 py-3 text-left font-chinese">分析??/th>
                <th className="px-4 py-3 text-left font-chinese">分析�價</th>
                <th className="px-4 py-3 text-left font-chinese">黃分析??/th>
                <th className="px-4 py-3 text-left font-chinese">分析</th>
        ...
        {skus.length === 0 && (
          <div className="p-8 text-center text-gray-500 font-chinese">
            尚未分析任分析SKU，分析?�設定分析體層級分析點分析?�自分析?�SKU??
          </div>
        )}
      </div>
    </div>
