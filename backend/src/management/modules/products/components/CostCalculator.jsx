...
const costMethods = [
  { key: 'fifo', label: 'FIFO ?�進分析?? },
  { key: 'lifo', label: 'LIFO 後進分析?? },
  { key: 'avg', label: '分析平分析' }
];
  ...
  return (
    <div className="glass rounded-2xl p-6">
      <div className="flex items-center mb-4">
        ...
        <h2 className="text-xl font-bold font-chinese">多分析分析分析/h2>
      </div>
        ...
        <table className="w-full">
          <thead className="bg-gradient-to-r from-[#cc824d] to-[#b3723f] text-white">
            <tr>
              ...
              <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">類分析</th>
              <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">分析</th>
              <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">分析?�本</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200">
            {mockStockRecords.map(r => (
              <tr key={r.id} className="hover:bg-white/30">
                ...
                <td className="px-6 py-4 font-chinese">{r.type==='in'?'?�庫':r.type==='out'?'?�庫':'-'}</td>
      ...
      <div className="font-chinese text-lg mt-4">
        <span className="mr-6">平分析?�本分析span className="font-bold text-purple-700">{result.avg}</span></span>
        <span>累分析?�本分析span className="font-bold text-purple-700">{result.total}</span></span>
      </div>
    </div>
  );
};
