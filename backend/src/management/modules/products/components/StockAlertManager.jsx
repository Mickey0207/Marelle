...
const mockAlerts = [
  { sku: 'TSHIRT-001-WH-M', name: '經典T???�色/M', stock: 3, safe: 5 },
  { sku: 'BOTTLE-001', name: '分析水壺', stock: 1, safe: 2 }
];
  ...
  return (
    <div className="glass rounded-2xl p-6">
      <div className="flex items-center mb-4">
        ...
        <h2 className="text-xl font-bold font-chinese">?�能低庫存警分析/h2>
      </div>
      <div className="mb-4 flex items-center space-x-2 font-chinese">
        ...
        />
        <span className="text-xs text-gray-400">（分析?�此分析將顯示警示分析</span>
      </div>
        ...
        <table className="w-full">
          <thead className="bg-gradient-to-r from-[#cc824d] to-[#b3723f] text-white">
            <tr>
              ...
              <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">分析庫分析</th>
              <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">安全水分析</th>
                ...
                <td className="px-6 py-4">
                  {item.stock < safeStock ? (
                    <span className="inline-flex items-center px-2 py-1 text-xs font-bold bg-red-100 text-red-700 rounded font-chinese">
                      <ExclamationTriangleIcon className="w-4 h-4 mr-1" />低於安全水分析
                    </span>
                  ) : (
                    <span className="inline-flex items-center px-2 py-1 text-xs font-bold bg-green-100 text-green-700 rounded font-chinese">分析��</span>
                  )}
                </td>
              </tr>
            ))}
