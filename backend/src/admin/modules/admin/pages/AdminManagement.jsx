...
import { ADMIN_STYLES, getStatusColor } from "@shared/adminStyles";
...
const AdminManagement = () => {
  ...
  const getUserDisplayName = (userId) => {
    ...
    return user ? user.displayName : '?�知?�戶';
  };
    ...
    >
      ...
      {onClick && (
        <div className="mt-2 text-right">
          <span className="text-xs text-blue-600 font-chinese">點擊查看詳細</span>
        </div>
      )}
    </div>
  );
      ...
      <div className="flex flex-col md:flex-row gap-4 md:items-center md:justify-between">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <CustomSelect
            options={[
              { value: '', label: '?�?��??? },
              { value: 'active', label: '?�用' },
              { value: 'disabled', label: '?�用' }
            ]}
            value={statusFilter}
      ...
      <div className="bg-white/50 backdrop-blur-sm rounded-xl overflow-hidden">
        <table className="min-w-full">
          <thead className="bg-gray-50/80">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">?�戶</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">角色</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">?�??/th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">?�後登??/th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">?��?</th>
            </tr>
          </thead>
                ...
                <tr key={user.id} className="hover:bg-white/20">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                      {roles.find(r => r.id === user.roleId)?.roleName || '?�知角色'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleString() : '從未?�入'}
                  </td>
  ...
  const renderRoleManagement = () => (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-gray-900 font-chinese">角色管�?</h3>
        <button
          onClick={() => {
            setEditingItem(null);
            setRoleFormData({
          ...
          <div key={role.id} className="bg-white/50 backdrop-blur-sm rounded-xl p-6 border border-white/30">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500">權�??�綴: {role.rolePrefix}</p>
              </div>
              <div className="flex space-x-2">
                <button
                  onClick={() => {
            ...
            <div className="space-y-3">
              <h5 className="text-sm font-medium text-gray-700">權�??�表:</h5>
  ...
  const renderSessionManagement = () => (
    <div className="p-6 space-y-6">
      <h3 className="text-lg font-semibold text-gray-900 font-chinese">活�??�話</h3>
      ...
      <div className="bg-white/50 backdrop-blur-sm rounded-xl overflow-hidden">
        <table className="min-w-full">
          <thead className="bg-gray-50/80">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">?�戶</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">?�入?��?</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">?�後活??/th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">?��??��?</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">?��?</th>
            </tr>
          </thead>
  ...
  const renderLoginLogs = () => (
    <div className="p-6 space-y-6">
      <h3 className="text-lg font-semibold text-gray-900 font-chinese">?�入?��?</h3>
      ...
      <div className="bg-white/50 backdrop-blur-sm rounded-xl overflow-hidden">
        <table className="min-w-full">
          <thead className="bg-gray-50/80">
            <tr>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">?�戶</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">類�?</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">?��?</th>
  ...
  return (
    <div className="bg-[#fdf8f2] min-h-screen p-6 space-y-6">
      {/* ?�面標�? */}
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-900 font-chinese">管�??�控?�台</h1>
      </div>

      {/* 快速�?作�???*/}
      <div className="glass rounded-2xl p-6">
        <h2 className="text-xl font-bold text-gray-900 font-chinese mb-4">快速�?�?/h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          >
            ...
            <span className="text-sm font-medium text-blue-900 font-chinese">管�??�管??/span>
          </button>
          <button
          ...
          >
            ...
            <span className="text-sm font-medium text-green-900 font-chinese">角色管�?</span>
          </button>
          <button
          ...
          >
            ...
            <span className="text-sm font-medium text-orange-900 font-chinese">?�話管�?</span>
          </button>
          <button
          ...
          >
            ...
            <span className="text-sm font-medium text-purple-900 font-chinese">?�入?��?</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* ?�近登??*/}
        <div className="glass rounded-2xl p-6">
          <h3 className="text-lg font-semibold text-gray-900 font-chinese mb-4">?�近登??/h3>
          <div className="space-y-3">
        ...
        {/* 活�??�話 */}
        <div className="glass rounded-2xl p-6">
          <h3 className="text-lg font-semibold text-gray-900 font-chinese mb-4">活�??�話</h3>
          <div className="space-y-3">
            {activeSessions.slice(0, 5).map((session) => (
              <div key={session.id} className="flex items-center justify-between p-3 bg-white/30 rounded-lg">
                <div>
                  ...
                  <p className="text-xs text-gray-500">?�後活?? {new Date(session.lastActivity).toLocaleString()}</p>
                </div>
