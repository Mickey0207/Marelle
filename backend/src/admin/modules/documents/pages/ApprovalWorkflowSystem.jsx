...
const ApprovalWorkflowSystem = () => {
  ...
  const loadApprovalData = async () => {
    try {
      ...
      // 模擬工�?流數??
      const mockWorkflows = [
        { id: 1, name: '?�購審批流�?', type: 'purchase', steps: 3 },
        { id: 2, name: '?�售審批流�?', type: 'sales', steps: 2 },
        { id: 3, name: '庫�?調整流�?', type: 'inventory', steps: 2 },
        { id: 4, name: '財�?審批流�?', type: 'finance', steps: 4 }
      ];
      
      setApprovals(pendingApprovals);
      setWorkflows(mockWorkflows);
    } catch (error) {
      console.error('載入審批資�?失�?:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-8">
      {/* Page Header */}
      <div>
        <h1 className="text-3xl font-bold text-gray-900 font-chinese">審批工�?�?/h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="stat-card glass p-6 rounded-2xl">
          <div className="flex items-center">
            ...
            <div className="ml-4 flex-1">
              <p className="text-sm font-medium text-gray-600 font-chinese">待審?�總??/p>
        ...
        <div className="stat-card glass p-6 rounded-2xl">
          <div className="flex items-center">
            ...
            <div className="ml-4 flex-1">
              <p className="text-sm font-medium text-gray-600 font-chinese">已審??/p>
        ...
        <div className="stat-card glass p-6 rounded-2xl">
          <div className="flex items-center">
            ...
            <div className="ml-4 flex-1">
              <p className="text-sm font-medium text-gray-600 font-chinese">已�?�?/p>
        ...
        <div className="stat-card glass p-6 rounded-2xl">
          <div className="flex items-center">
            ...
            <div className="ml-4 flex-1">
              <p className="text-sm font-medium text-gray-600 font-chinese">?��?審批</p>
      ...
      <div className="glass p-2 rounded-2xl">
        <div className="flex space-x-1">
          {[
            { id: 'pending', name: '待審??, count: statistics.pending },
            { id: 'approved', name: '已審??, count: statistics.approved },
            { id: 'rejected', name: '已�?�?, count: statistics.rejected },
            { id: 'overdue', name: '?��?', count: statistics.overdue }
          ].map(tab => (
            <button
              key={tab.id}
      ...
      <div className="glass p-6 rounded-2xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <SearchableSelect
            ...
            options={[
              { value: 'all', label: '?�?�工作�?' },
              { value: 'purchase', label: '?�購審批流�?' },
              { value: 'sales', label: '?�售審批流�?' },
              { value: 'inventory', label: '庫�?審批流�?' },
              { value: 'finance', label: '財�?審批流�?' }
            ]}
            placeholder="?��?工�?�?
            searchPlaceholder="?��?工�?�?.."
            allowClear
      ...
      <div className="glass rounded-2xl overflow-visible">
        ...
        {filteredApprovals.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2 font-chinese">沒�??�到審批?�目</h3>
            <p className="text-gray-500 font-chinese">?��?篩選條件下�??��?要�??��?審批</p>
          </div>
        )}
      </div>
      ...
      {/* 工�?流�?�?*/}
      <div className="glass p-6 rounded-2xl">
        <h3 className="text-lg font-semibold text-gray-900 mb-6 font-chinese">工�?流�?�?/h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {workflows.map(workflow => (
            <div key={workflow.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
              ...
              <p className="text-sm text-gray-500 mb-3 font-chinese">{workflow.steps} ?�審?�步�?/p>
