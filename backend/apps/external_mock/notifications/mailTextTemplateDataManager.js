...
export function getMailMessageTextTemplate() {
  return '嗨 {{ user.name }}，您的訂單 {{ order.id }} 已出貨，謝謝！\n\n如有任何問題，歡迎隨時聯繫我們的客服。';
}

// Simple localStorage-based template store for Mail
const KEY = '__mock_mail_text_templates__';
function load() {
...
export function createTemplate(partial) {
  ...
  const item = {
    ...
    category: '',
    tags: [],
    subject: 'Marelle 通知：訂單 {{ order.id }} 已出貨',
    content: getMailMessageTextTemplate(),
    headerImageUrl: '',
    signatureText: '—\nMarelle 客服團隊',
    signatureImageUrl: '',
    ...partial,
...
export function duplicateTemplate(id) {
  ...
  if (!src) return null;
  const copy = { ...src, id: `MT-${Date.now()}`, name: `${src.name || '未命名'} (複製)`, updatedAt: Date.now() };
...
export function renderMailFieldsWithContext({ subject, content, signatureText }, ctx) {
  return {
    subject: renderTemplateWithContext(subject || '', ctx),
    content: renderTemplateWithContext(content || '', ctx),
    signatureText: renderTemplateWithContext(signatureText || '', ctx),
  };
}
