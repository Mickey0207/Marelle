...
import {
  ...
  duplicateTemplate,
  publishTemplate,
  renderMailHtmlWithContext,
} from '../../../../external_mock/notifications/mailHtmlTemplateDataManager';

const MailHtmlMessage = () => {
  // 安全 HTML 預覽（簡單白名單：常用文字與連結、區塊）
  const HtmlPreview = ({ subject = '', html = '' }) => {
    // 為避免 XSS，此處僅針對常見 inline 樣式與基本標籤做簡易白名單；更完整方案建議引入 DOMPurify 類工具
    const sanitize = (raw) => {
      try {
        const allowedTags = ['a','p','div','span','strong','b','em','i','u','h1','h2','h3','h4','h5','h6','ul','ol','li','br','hr','img','table','thead','tbody','tr','td','th'];
        const allowedAttrs = ['href','style','src','alt','width','height','target','rel'];
        const doc = new DOMParser().parseFromString(raw, 'text/html');
        const walk = (node) => {
          if (node.nodeType === 1) {
            const tag = node.tagName.toLowerCase();
            if (!allowedTags.includes(tag)) { node.replaceWith(...Array.from(node.childNodes)); return; }
            // 過濾屬性
            Array.from(node.attributes).forEach(attr => {
              const name = attr.name.toLowerCase();
              if (!allowedAttrs.includes(name)) node.removeAttribute(attr.name);
              // 移除 javascript: 協議
    ...
    return (
      <div className="w-full">
        <div className="w-full max-w-3xl rounded-2xl border border-gray-200 overflow-hidden bg-white shadow">
          <div className="bg-gray-800 text-white px-4 py-2 text-sm font-medium">Email HTML 預覽</div>
          <div className="p-5 space-y-3">
            <div className="text-lg font-semibold text-gray-900 break-words">{subject || '（無主旨）'}</div>
            <div className="h-px bg-gray-200" />
            <div className="prose prose-sm max-w-none" dangerouslySetInnerHTML={{ __html: safe || '<p class="text-gray-400">（尚未輸入 HTML 內容）</p>' }} />
          </div>
        </div>
      </div>
    );
  ...
  const usedPaths = useMemo(() => {
    ...
    const set = new Set();
    let m;
    [subject, html].forEach((txt) => { while ((m = re.exec(txt))) { if (m[1]) set.add(m[1]); } });
  ...
  useEffect(() => {
    if (current) {
      ...
      setForm({ name: '', category: '', description: '', tags: [], subject: '', html: '' });
    }
  }, [current]);
  ...
  const handleCreate = () => {
    const item = createTemplate({ name: '新 HTML 郵件範本', description: '' });
  ...
  const insertAtCursor = (ref, valueToInsert) => {
    ...
    const key = el === subjectRef.current ? 'subject' : 'html';
    setForm((p) => ({ ...p, [key]: next }));
    requestAnimationFrame(() => { el.focus(); const pos = start + valueToInsert.length; el.setSelectionRange(pos, pos); });
  };

  // 發送測試（模擬）
  const [testForm, setTestForm] = useState({ email: '' });
  const handleSendTest = () => {
    setSending(true);
    setSendResult(null);
    const rendered = renderMailHtmlWithContext({ subject: form.subject, html: form.html }, previewCtx);
    setTimeout(() => { setSending(false); setSendResult({ success: true, message: '測試郵件已模擬送出', rendered }); }, 700);
  };
    ...
    <div className={`${ADMIN_STYLES.pageContainer}`}>
      <div className={`${ADMIN_STYLES.contentContainer}`}>
        <div className="flex items-center justify-between">
          <div>
            <h1 className={ADMIN_STYLES.pageTitle}>Mail HTML</h1>
            <p className={ADMIN_STYLES.pageSubtitle}>集中管理 Email HTML 範本（用於推銷官網）</p>
          </div>
          ...
          <div className="col-span-1">
            <div className={`${ADMIN_STYLES.glassCard} space-y-4`}>
              <div className="flex gap-3">
                <input placeholder="搜尋名稱、描述、分類、主旨、標籤" value={keyword} onChange={(e) => setKeyword(e.target.value)} className={`${ADMIN_STYLES.input}`} />
              </div>
            ...
            <div className={`${ADMIN_STYLES.glassCard} space-y-6`}>
              {!current ? (
                ...
                <>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2 gap-3">
                      <label className="block text-sm font-medium text-gray-700 font-chinese">HTML 內容</label>
                      <div className="flex items-center gap-2">
                        <div className="text-xs text-gray-500">可用變數：{'{{user.name}}'}, {'{{order.number}}'}, {'{{shipment.tracking}}'}</div>
                        <button className={ADMIN_STYLES.btnGhost} onClick={() => setShowVarPicker(true)}>插入變數</button>
                      </div>
                    </div>
                    <textarea ref={htmlRef} className={`${ADMIN_STYLES.input} min-h-[260px] font-mono text-[13px]`} value={form.html} onChange={(e) => setForm((p) => ({ ...p, html: e.target.value }))} placeholder="輸入 Email 內嵌 HTML（建議 inline-style）" />
                  </div>
          ...
          {previewActiveTab === 'preview' && (
            <div className="grid grid-cols-12 gap-6 items-start">
              <div className="col-span-7">
                {(() => {
                  const rendered = renderMailHtmlWithContext({ subject: form.subject, html: form.html }, previewCtx);
                  return <HtmlPreview subject={rendered.subject} html={rendered.html} />;
                })()}
              </div>
              ...
              <div className="col-span-12">
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <SearchableSelect options={getPresetOptions()} value={previewPreset} onChange={(val) => { setPreviewPreset(val); setPreviewCtx(getPresetContext(val)); }} placeholder="選擇情境" />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button className={ADMIN_STYLES.secondaryButton} onClick={() => { setPreviewPreset('default'); setPreviewCtx(getPresetContext('default')); }}>重置</button>
                    </div>
                  </div>
                </div>
              </div>
              ...
              .map((v) => (
                <div key={v.token} className="flex items-center justify-between p-4 rounded-xl border border-gray-200/80 bg-white/80 hover:bg-gray-50/60 transition">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button className={ADMIN_STYLES.primaryButton} onClick={() => insertAtCursor(htmlRef, v.token)}>插入 HTML</button>
                  </div>
                </div>
              ))}
          </div>
