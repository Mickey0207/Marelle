...
import {
  ...
  duplicateTemplate,
  publishTemplate,
  renderSmsWithContext,
} from '../../../../external_mock/notifications/smsTemplateDataManager';

const SmsMessage = () => {
  // 手機簡訊預覽
  const SmsPhonePreview = ({ content = '', from = 'Marelle' }) => {
    const now = new Date();
    const ts = now.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    const text = content?.trim() ? content : '（尚未輸入內容）';
    return (
      <div className="w-full">
        <div className="mx-auto w-full max-w-[320px] rounded-[36px] border border-gray-300 bg-white shadow relative overflow-hidden">
          {/* notch */}
          <div className="absolute left-1/2 -translate-x-1/2 top-0 h-6 w-32 bg-black/10 rounded-b-2xl" />
          {/* header bar */}
          <div className="pt-6" />
          <div className="px-4 py-2 border-b border-gray-200 text-center text-sm font-medium text-gray-800">訊息</div>
          {/* chat area */}
          <div className="bg-gray-100 h-[480px] overflow-y-auto p-3 space-y-2">
            {/* incoming bubble */}
          ...
          {/* bottom bar */}
          <div className="bg-white border-t border-gray-200 px-3 py-2 text-gray-400 text-sm">簡訊輸入列（預覽）</div>
        </div>
      </div>
    );
  };
  ...
  const handleCreate = () => { const item = createTemplate({ name: '新簡訊範本', description: '' }); const next = getTemplates(); setList(next); setSelectedId(item.id); };
  ...
  const insertAtCursor = (txt) => {
    ...
    if (!el) { setForm((p) => ({ ...p, content: (p.content || '') + txt })); return; }
    ...
    requestAnimationFrame(() => { el.focus(); const pos = start + txt.length; el.setSelectionRange(pos, pos); });
  };

  // 測試
  const [testForm, setTestForm] = useState({ phone: '' });
  const handleSendTest = () => {
    setSending(true);
    setSendResult(null);
    const rendered = renderSmsWithContext(form.content, previewCtx);
    setTimeout(() => { setSending(false); setSendResult({ success: true, message: '測試簡訊已模擬送出', rendered }); }, 700);
  };
    ...
    <div className={`${ADMIN_STYLES.pageContainer}`}>
      <div className={`${ADMIN_STYLES.contentContainer}`}>
        <div className="flex items-center justify-between">
          <div>
            <h1 className={ADMIN_STYLES.pageTitle}>SMS</h1>
            <p className={ADMIN_STYLES.pageSubtitle}>集中管理簡訊文字範本</p>
          </div>
          ...
          <div className="col-span-1">
            <div className={`${ADMIN_STYLES.glassCard} space-y-4`}>
              <div className="flex gap-3">
                <input placeholder="搜尋名稱、描述、分類、標籤" value={keyword} onChange={(e) => setKeyword(e.target.value)} className={`${ADMIN_STYLES.input}`} />
              </div>
              ...
              {!current ? (
                ...
                <>
                  ...
                  <div>
                    ...
                    <SearchableSelect options={[{label:'otp',value:'otp'},{label:'order',value:'order'},{label:'logistics',value:'logistics'},{label:'promo',value:'promo'},{label:'notice',value:'notice'}]} value={form.tags} onChange={(v) => setForm((p) => ({ ...p, tags: v }))} multiple={true} placeholder="新增或選擇標籤" />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea ref={contentRef} className={`${ADMIN_STYLES.input} min-h-[160px]`} value={form.content} onChange={(e) => setForm((p) => ({ ...p, content: e.target.value }))} placeholder="輸入簡訊內容（70/160 字限制視編碼與長簡訊拆分）" />
                  </div>
      ...
      <GlassModal isOpen={showPreview} onClose={() => setShowPreview(false)} title="預覽" size={modalSize}>
        <div className="space-y-6 p-6">
          <TabNavigation mode="controlled" activeKey={previewActiveTab} onTabChange={(t) => setPreviewActiveTab(t.key || t.label)} layout="left" className="px-2" tabs={[{ key: 'preview', label: '預覽與發送' }, { key: 'context', label: '情境與欄位' }]} />

          {previewActiveTab === 'preview' && (
            <div className="grid grid-cols-12 gap-6 items-start">
              <div className="col-span-6">
                <SmsPhonePreview content={renderSmsWithContext(form.content, previewCtx)} />
              </div>
              <div className="col-span-6">
                <div className="text-sm text-gray-700 font-medium mb-3">發送測試</div>
                <div className="space-y-3">
                  <div>
                    <label className="block text-xs text-gray-600 mb-1">測試收件者（手機號碼）</label>
                    <input className={ADMIN_STYLES.input} value={testForm.phone} onChange={(e) => setTestForm((p) => ({ ...p, phone: e.target.value }))} placeholder="09xx-xxx-xxx" />
                  </div>
                  <div className="text-xs text-gray-500">不會真正發送簡訊，只做本地模擬</div>
