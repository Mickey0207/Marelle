            ...
            <tbody className="divide-y divide-gray-200/60">
              {currentOrders.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50/40 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-900">{(order.itemCount ?? order.items ?? 0)} 項商品</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm font-medium text-gray-900">{Number((order.totalAmount ?? order.amount) || 0).toLocaleString()}</div>
                  </td>
                  <td className="px-6 py-4">
                    {getStatusBadge(order.status)}
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-900">{order.createdAt || order.date || '-'}</div>
                  </td>
