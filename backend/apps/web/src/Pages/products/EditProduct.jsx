...
const EditProduct = () => {
  ...
  promotionLabel: p.promotion_label || '',
  promotionLabelBgColor: p.promotion_label_bg_color || '#CC824D',
  promotionLabelTextColor: p.promotion_label_text_color || '#FFFFFF',
  productTagBgColor: p.product_tag_bg_color || '#CC824D',
  productTagTextColor: p.product_tag_text_color || '#FFFFFF',
  autoHideWhenOOS: !!p.auto_hide_when_oos,
  enablePreorder: !!p.enable_preorder,
  preorderStartAt: p.preorder_start_at ? new Date(p.preorder_start_at).toISOString().slice(0,16) : '',
  preorderEndAt: p.preorder_end_at ? new Date(p.preorder_end_at).toISOString().slice(0,16) : '',
  preorderMaxQty: (p.preorder_max_qty ?? ''),
      price: p.price ?? '',
                    ...
                    <div className="flex flex-wrap gap-2 mb-2">
                      {productData.tags.map(tag => (
                        <span
                          ...
                          style={{ background: productData.productTagBgColor, color: productData.productTagTextColor }}
                        >
                          {tag}
                    ...
                    <div className="space-y-3">
                      <label className="inline-flex items-center gap-2">
                        <input
                          ...
                          checked={productData.autoHideWhenOOS}
                          onChange={(e)=>handleInputChange('autoHideWhenOOS', e.target.checked)}
                        />
                      ...
                      <div className="flex flex-col gap-3">
                        <label className="inline-flex items-center gap-2">
                          <input
                            ...
                            id="enablePreorder"
                            className="h-4 w-4 text-[#cc824d] border-gray-300 rounded focus:ring-[#cc824d]"
                            checked={productData.enablePreorder}
                            onChange={(e)=>handleInputChange('enablePreorder', e.target.checked)}
                          />
                        ...
                        {productData.enablePreorder && (
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                              <label className="block text-xs text-gray-600 mb-1" htmlFor="preorderStartAt">預購開始時間</label>
                              <input
                                id="preorderStartAt"
                                type="datetime-local"
                                value={productData.preorderStartAt}
                                onChange={(e)=>handleInputChange('preorderStartAt', e.target.value)}
                                className={ADMIN_STYLES.input}
                              />
                            </div>
                            <div>
                              <label className="block text-xs text-gray-600 mb-1" htmlFor="preorderEndAt">預購結束時間</label>
                              <input
                                id="preorderEndAt"
                                type="datetime-local"
                                value={productData.preorderEndAt}
                                onChange={(e)=>handleInputChange('preorderEndAt', e.target.value)}
                                className={ADMIN_STYLES.input}
                              />
                            </div>
                            <div>
                              <label className="block text-xs text-gray-600 mb-1" htmlFor="preorderMaxQty">最大預購數</label>
                              <input
                                id="preorderMaxQty"
                                type="number"
                                min="0"
                                value={productData.preorderMaxQty}
                                onChange={(e)=>handleInputChange('preorderMaxQty', e.target.value)}
                                className={ADMIN_STYLES.input}
                                placeholder="例如 100"
                              />
                            </div>
