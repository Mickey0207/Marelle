...
const costMethods = [
  { key: 'fifo', label: 'FIFO ?àÈÄ≤Â??? },
  { key: 'lifo', label: 'LIFO ÂæåÈÄ≤Â??? },
  { key: 'avg', label: '?†Ê?Âπ≥Â?' }
];
  ...
  return (
    <div className="glass rounded-2xl p-6">
      <div className="flex items-center mb-4">
        ...
        <h2 className="text-xl font-bold font-chinese">Â§öÊ??¨Ë?ÁÆ?/h2>
      </div>
      ...
      <div className="overflow-x-auto mb-6">
        <table className="w-full">
          <thead className="bg-gradient-to-r from-[#cc824d] to-[#b3723f] text-white">
            <tr>
              <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">?•Ê?</th>
              <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">È°ûÂ?</th>
              <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">?∏È?</th>
              <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">?Æ‰??êÊú¨</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200">
            {mockStockRecords.map(r => (
              <tr key={r.id} className="hover:bg-white/30">
                ...
                <td className="px-6 py-4 font-chinese">{r.type==='in'?'?•Â∫´':r.type==='out'?'?∫Â∫´':'-'}</td>
      ...
      <div className="font-chinese text-lg mt-4">
        <span className="mr-6">Âπ≥Â??êÊú¨Ôº?span className="font-bold text-purple-700">{result.avg}</span></span>
        <span>Á¥ØË??êÊú¨Ôº?span className="font-bold text-purple-700">{result.total}</span></span>
      </div>
    </div>
  );
};
