...
const WarehouseManager = () => {
  ...
  const handleDelete = id => {
    if (window.confirm('確定要刪除此倉庫？')) setWarehouses(ws => ws.filter(w => w.id !== id));
  };
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold flex items-center font-chinese">
          <BuildingStorefrontIcon className="w-6 h-6 mr-2 text-blue-400" />倉庫管理
        </h2>
        <button className="btn-primary flex items-center" onClick={openAdd}><PlusIcon className="w-5 h-5 mr-1" />新增倉庫</button>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-gradient-to-r from-[#cc824d] to-[#b3723f] text-white">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">代碼</th>
                <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">地址</th>
                <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">管理員</th>
                <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">電話</th>
                <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">啟用</th>
                <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">預設</th>
                <th className="px-6 py-4 text-left text-sm font-semibold font-chinese">操作</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {warehouses.map(w => (
                <tr key={w.id} className="hover:bg-white/30">
                  ...
                  <td className="px-6 py-4">{w.isActive ? '是' : '否'}</td>
                  <td className="px-6 py-4">{w.isDefault ? '是' : '否'}</td>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          <form className="bg-white rounded-xl p-8 w-full max-w-md space-y-4 shadow-lg relative" onSubmit={handleSubmit}>
            ...
            <label className="block font-chinese">地址<input className="input" name="address" value={form.address} onChange={handleChange} /></label>
            <label className="block font-chinese">管理員<input className="input" name="manager" value={form.manager} onChange={handleChange} /></label>
            <label className="block font-chinese">電話<input className="input" name="phone" value={form.phone} onChange={handleChange} /></label>
            <label className="inline-flex items-center mr-4 font-chinese"><input type="checkbox" name="isActive" checked={form.isActive} onChange={handleChange} /> 啟用</label>
            <label className="inline-flex items-center font-chinese"><input type="checkbox" name="isDefault" checked={form.isDefault} onChange={handleChange} /> 預設</label>
            <div className="flex justify-end space-x-2 mt-4">
              <button type="button" className="btn-secondary" onClick={closeForm}>取消</button>
              <button type="submit" className="btn-primary">{editing ? '儲存' : '新增'}</button>
            </div>
          </form>
        </div>
      )}
