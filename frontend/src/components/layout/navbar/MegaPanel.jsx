...
const MegaPanel = ({
  ...
  const currentLevel2 = level2List.find((c) => c.id === activeLevel2) || null;
  const level3List = currentLevel2?.children || [];
  const currentLevel3 = level3List.find((c) => c.id === activeLevel3) || null;
  const level4List = currentLevel3?.children || [];
        ...
        <div className="flex items-center mb-8">
          <nav className="flex items-center space-x-2 text-base font-chinese">
            ...
            {currentLevel3 && (
              <>
                ...
                <button
                  ...
                  className="transition-colors hover:text-primary-btn"
                  style={{ color: panelMode === 'level3' ? '#CC824D' : '#666666' }}
                >
                  {currentLevel3.name}
                </button>
              </>
            ...
            {currentLevel4 && (
              <>
                <span style={{ color: '#999999' }}>/</span>
                <span style={{ color: '#CC824D', fontWeight: 500 }}>{currentLevel4.name}</span>
              </>
            )}
          </nav>
        </div>
                  ...
                  <div key={l2.id} className="group">
                    <Link to={l2.href} onClick={resetAll} className="block">
                      <div className="w-full aspect-[4/3] rounded-xl overflow-hidden mb-2.5 bg-white/30 ring-1 ring-white/40 backdrop-blur-sm relative shadow-sm cursor-pointer">
                        ...
                        />
                        {Array.isArray(l2.children) && l2.children.length > 0 && (
                          <span
                      ...
                      <div
                        className="text-[11px] text-center font-chinese tracking-wide hover:text-primary-btn transition-colors"
                        style={{ color: '#444' }}
                      >
                        {l2.name}
                      </div>
                    ...
                    <Link to={l3.href} onClick={resetAll} className="block">
                      <div className="w-full aspect-[4/3] rounded-xl overflow-hidden mb-2.5 bg-white/30 ring-1 ring-white/40 backdrop-blur-sm relative shadow-sm cursor-pointer">
                        <img
                          ...
                          className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                        />
                        {Array.isArray(l3.children) && l3.children.length > 0 && (
                          <span
                            className="absolute bottom-3 right-3 text-xs px-3 py-1.5 rounded-full bg-primary-btn text-white tracking-wider font-medium shadow-lg hover:bg-opacity-90 transition-all z-10"
                            onClick={(e) => {
                              e.preventDefault();
                    ...
                    <Link to={l4.href} onClick={resetAll} className="block">
                      <div className="w-full aspect-[4/3] rounded-xl overflow-hidden mb-2.5 bg-white/30 ring-1 ring-white/40 backdrop-blur-sm relative shadow-sm cursor-pointer">
                        <img
                          ...
                          className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                        />
                        {Array.isArray(l4.children) && l4.children.length > 0 && (
                          <span
                            className="absolute bottom-3 right-3 text-xs px-3 py-1.5 rounded-full bg-primary-btn text-white tracking-wider font-medium shadow-lg hover:bg-opacity-90 transition-all z-10"
                            onClick={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              setActiveLevel4(l4.id);
                              setPanelMode('level5');
                            }}
                          >
                            更多
